...
export function TransferInventoryForm({
  ...
  return (
    <div className="space-y-4">
      ...
      {origin && destination && (
        <div className="space-y-4 mt-4">
          ...
          <div className="space-y-3 max-h-[40vh] overflow-y-auto p-1">
            {items.length > 0 && (
              <div className="grid grid-cols-[1fr_80px_80px_40px] md:grid-cols-[1fr_100px_100px_40px] gap-2 mb-2 text-xs md:text-sm font-medium text-muted-foreground px-1">
                <div>Produto</div>
            ...
            {items.map((item) => {
              ...
              return (
                <div
                  key={item.id}
                  className="grid grid-cols-[1fr_80px_80px_40px] md:grid-cols-[1fr_100px_100px_40px] gap-2 items-center"
                >
                  <Select
                  ...
                  <Input
                    ...
                    value={item.quantity || ''}
                    onChange={(e) =>
                      handleUpdate(item.id, 'quantity', e.target.value ? Number(e.target.value) : 0)
                    }
      ...
      <div className={`pt-4 flex ${isDialog ? 'justify-end gap-2' : 'flex-col gap-2'}`}>
        {onCancel && (
